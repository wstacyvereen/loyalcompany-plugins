...
class Module extends Module_Base {
	...
	protected static function get_templates( $args = [] ) {
		...
		return self::elementor()->templates_manager->get_source( 'local' )->get_items( $args );
	}

	/**
	 * @since 2.0.0
	 */
	protected static function empty_templates_message() {
		...
		if ( empty( $templates ) ) {
			...
			$object->add_control(
				...
				[
					...
					'raw' 	=> self::empty_templates_message(),
					'condition'	=> $args['condition'],
				]
			);
	...
	public static function render_template_content( $id ) {
		...
		$template = self::elementor()->frontend->get_builder_content_for_display( $id );
